<template>
  <form @submit.prevent>
    <h2 class="text-white text-4xl">Create Hotel</h2>
    <my-input
      v-focus
      v-model:value="hotel.title"
      type="text"
      placeholder="Title"
      required
      minlength="1"
    ...
    <textarea
      ...
      cols="30"
      rows="10"
      v-bind:value="hotel.address"
      @input="hotel.address = $event.target.value"
      placeholder="Address"
      required
      minlength="10"
      class="input-modal"
...
export default {
  ...
  methods: {
    createHotel() {
      ...
      this.$emit("create", this.hotel, this.file);
